...
#################################
#### Plot output from fastPHASE with order based on gene tree
#### DLF 08Feb2013
#################################
...
#### to make this flexible for bigger regions, just modify the gene.star and gene.end values...
#### although this will mess up your "region of interest bars on the plot, but meh...

plotFastphase.genetree <- function(haplotypes,pheno.file,gene.start,gene.end,meta.file,plot.pdf.file){
  ...
  snps <- snps[,snps.pos%in%c(gene.start:gene.end)]
  snps.dist <- dist(snps)
...
plotFastphase.snp <- function(haplotypes,pheno.file,gene.start,gene.end,meta.file,plot.pdf.file,snpbp){
  #pheno.file="monica.meristem.length.csv"
  #gene.start =  up.genes$start
  #gene.end =  up.genes$stop
  #meta.file = "/Volumes/daniele.filiault/Documents/006.FLC.evolution/001.FLC.comparative/001.sequences/001.DNA/005.peijin.fall2011/fastPHASE.functions/call_method_75_info.csv"
  #plot.pdf.file = "KUK.with.SNP.pdf"
  #snpbp = 22244990
  ...
  snps.clust.dend <- as.dendrogram(snps.clust)
  haplotypes.snp.sort <- haplotypes[snps.clust$order,]
  snps.sort <- snps[snps.clust$order,]
  snps.sort.cand <- snps.sort[,snps.pos==snpbp]
  if (length(snps.sort.cand)==0){print("Wait a minute, this SNP isn't in the data set!")}
  ...
  # element 3. haplotypes
  par(mai=c(0.1,0.8,0.1,0.1),fig=c(0.0,1,0.16,0.5),new=TRUE)
  x <- 1:nrow(haplotypes.snp.sort)
  y <- 1:ncol(haplotypes.snp.sort)
  image(x,y,as.matrix(haplotypes.snp.sort),axes=FALSE,xlab="",ylab="region of interest",col=col.up.fig,xaxs="i")
  interest.gene <- c(gene.start:gene.end)
  ...
  #element 4. phenotypes
  par(mai=c(0.1,0.8,0.1,0.1),fig=c(0,1,0.5,0.7),new=TRUE)
  plot(all.info.sort[,colnames(all.info.sort)==pheno.name],xaxt="n",ylab=pheno.name,xaxs="i",yaxs="i",xlab="")

  #element 4.5 snps
  par(mai=c(0.1,0.8,0.1,0.1),fig=c(0,1,0.7,0.80),new=TRUE)
  plot(snps.sort.cand,xaxt="n",ylab="SNP",xaxs="i",yaxs="i",xlab="", ylim=c(-0.2,1.2),yaxt="n")
...
plotFastphase.subset.snp <- function(haplotypes,pheno.file,gene.start,gene.end,meta.file,plot.pdf.file,snpbp,subset){
  #pheno.file="fall2011.flowering.csv"
  #gene.start =  up.genes$start
  #gene.end =  up.genes$stop
  #meta.file = "/Volumes/daniele.filiault/Documents/006.FLC.evolution/001.FLC.comparative/001.sequences/001.DNA/005.peijin.fall2011/fastPHASE.functions/call_method_75_info.csv"
  #plot.pdf.file = "KUK.with.SNP.pdf"
  #snpbp = 196050
  #subset is vector of accessions names for subsetting - i.e. Sweden field lines
  ...
  for (up in c(1:ncol(snps))){
    snps[,up]<-as.numeric(snps[,up])}
  snps <- snps-1
  snps <- snps[rownames(snps)%in%subset,]

  # here is where you could subset snps to cluster using different SNP sets:
  snps.dist <- dist(snps)
  snps.clust <-hclust(snps.dist)
  snps.clust.dend <- as.dendrogram(snps.clust)
  haplotypes <- haplotypes[rownames(haplotypes)%in%subset,]
  haplotypes.snp.sort <- haplotypes[snps.clust$order,]
  snps.sort <- snps[snps.clust$order,]
  snps.sort.cand <- snps.sort[,snps.pos==snpbp]
  if (length(snps.sort.cand)==0){print("Wait a minute, this SNP isn't in the data set!")}
...
plotFastphase.subset <- function(haplotypes,pheno.file,gene.start,gene.end,meta.file,plot.pdf.file,subset){
  #pheno.file="fall2011.flowering.csv"
  #gene.start =  up.genes$start
  #gene.end =  up.genes$stop
  #meta.file = "/Volumes/daniele.filiault/Documents/006.FLC.evolution/001.FLC.comparative/001.sequences/001.DNA/005.peijin.fall2011/fastPHASE.functions/call_method_75_info.csv"
  ...
  #element 4. phenotypes
  par(mai=c(0.1,0.8,0.1,0.1),fig=c(0,1,0.6,0.8),new=TRUE)
  plot(all.info.sort[,colnames(all.info.sort)==pheno.name],xaxt="n",ylab=pheno.name,xaxs="i",yaxs="i",xlab="")
  
  #element 5.tree
  par(mai=c(0.1,0.8,0.1,0.1),fig=c(0,1,0.8,1),new=TRUE)
  plot(snps.clust.dend,leaflab="none",ann=FALSE,yaxt="n",xlim=c(0,max(x)),xaxs="i",yaxs="i")
...
# candidate is a data frame with ecotype_id in first column and snp in second
# these must be the colnames!)


plotFastphase.genetree.candidateSNP <- function(haplotypes,pheno.file,gene.start,gene.end,meta.file,plot.pdf.file,candidate){
    ...
    ### step two: get latitude, longitude, phenotype, candidateSNP info for accessions and put in appropriate order
    phenos <- read.csv(pheno.file,stringsAsFactors=FALSE)
    pheno.name <- colnames(phenos)[2]
    colnames(phenos)[1]<-"ecotype_id"
    meta <- read.csv(meta.file, stringsAsFactors=FALSE)
    ...
    all.cand <- merge(all.info,candidate,all.x=TRUE)
    all.cand.sort <- all.cand[snps.clust$order,]
    ...
    #5.tree
    # new 5.  replacing tree with candidate SNP information
    #par(mai=c(0.1,0.8,0.1,0.1),fig=c(0,1,0.80,1),new=TRUE)
    #plot(snps.clust.dend,leaflab="none",ann=FALSE,yaxt="n",xlim=c(0,max(x)),xaxs="i",yaxs="i")
    par(mai=c(0.1,0.8,0.1,0.1),fig=c(0,1,0.80,1),new=TRUE)
    plot(all.cand.sort$snp,xaxt="n",ylab="candidate SNP",xaxs="i",yaxs="i",xlab="",ylim=c(min(all.cand.sort$snp,na.rm=TRUE)-0.2,max(all.cand.sort$snp,na.rm=TRUE)+0.2))
...
plotFastphase.genetree.candidateSNP.swedes <- function(haplotypes,pheno.file,gene.start,gene.end,meta.file,plot.pdf.file,candidate){
    ...
    #### subset these guys for swedish accessions only
    all.cand.sort <- all.cand.sort[all.info.sort$country=="SWE",]
    haplotypes.snp.sort <- haplotypes.snp.sort[all.info.sort$country=="SWE",] 
	all.info.sort<-all.info.sort[all.info.sort$country=="SWE",]
